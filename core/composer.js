...
function caretInfos(block) {
    'use strict';
    var start, end, isEnd, selRange, testRange, sel, child, i;

    sel = window.getSelection();
    selRange = sel.getRangeAt(0);
    testRange = selRange.cloneRange();
    child = testRange.endContainer;
    i = 0;
    
    if (child === block) {
        child = block.lastChild;
    }
    while( (child = child.previousSibling) != null ) {
        if (child.nodeName !== 'BR') i++;
    }
    ...
    // 4 pour le nombre de caractères dans '<br>'
    return {start: start + 4 * i || 0, end: end + 4 * i || 0, isEnd: isEnd};
    //voir le cas où l'on traite plusieurs lignes d'un coup
}
...
function setCaret(block, caret) {
    ...
    if (nodes.length === 1) {
        ...
        //bloc multilignes, la selection (caret.end != caret.start) ne fonctionne pas encore
        console.log(caret);
        
        length = 0;
        nb = block.childNodes.length;
        
        for (i = 0; i < nb; i++) {
            node = block.childNodes[i];
            length += node.nodeType === 3 ? node.length : 4; //4 = <br> length
            if (length >= caret.start) break;
        }
        console.log(i);
        node = block.childNodes[i];
        caret = {start: node.length, end: node.length};
    }
                ...
                while ((array = regexImg.exec(ml)) !== null) {
                    ...
                    for (i = 0, nb = opts.length; i < nb; i++) {
                        ...
                        if (i === nb - 1) { //last opt is link, always.
                            ...
                            if (['25', '33', '50', '75'].indexOf(opt) !== -1) {
                                img.classList.add('imgWidth' + opt);
                            }
                        } else if (opt !== '') {
                            if (thisCase === 0) {
